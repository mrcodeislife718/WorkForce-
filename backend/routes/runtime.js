...
function taskPayload(body) {
  ...
  return {
    trace_id: String(body.trace_id || randomUUID()),
    title,
    instructions,
...
async function enqueue(deployment, payload) {
  ...
  if (deployment.runtime_configuration?.quarantined) {
    throw new Error('Digital employee deployment is quarantined by the Workforce control plane.');
  }
...
router.post('/deployments/:deploymentId/tasks', deploymentTokenAuth, async (req, res) => {
  try {
    ...
    return res.status(202).json({ job_id: job.id, trace_id: job.payload.trace_id, status: job.status });
  } catch (error) {
...
router.get('/jobs/:id', auth, async (req, res) => {
  try {
    ...
    return res.json({ trace_id: job.payload?.trace_id || null, job, task_run: taskRun });
  } catch (error) {
